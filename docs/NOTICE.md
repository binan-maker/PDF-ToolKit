# NOTICE

Paperly
Copyright 2024-2025 Paperly Contributors

This product includes software developed by third parties.

---

## Open Source Libraries Used

### PdfBox-Android
- **License**: Apache License 2.0
- **Source**: https://github.com/TomRoush/PdfBox-Android
- **Copyright**: Copyright 2017-2024 Tom Roush

PdfBox-Android is a port of Apache PDFBox for Android.
Licensed under the Apache License, Version 2.0.

---

### Apache PDFBox
- **License**: Apache License 2.0
- **Source**: https://pdfbox.apache.org/
- **Copyright**: Copyright 2002-2024 The Apache Software Foundation

Apache PDFBox is an open source Java tool for working with PDF documents.
Licensed under the Apache License, Version 2.0.

---

### Google ML Kit Text Recognition
- **License**: Apache License 2.0 (SDK), Google Mobile Services Terms (Models)
- **Source**: https://developers.google.com/ml-kit/vision/text-recognition
- **Copyright**: Copyright Google LLC

Google ML Kit provides on-device machine learning for mobile apps.
The SDK is licensed under Apache 2.0. Usage of the text recognition
models is subject to the Google APIs Terms of Service.

---

### uCrop (Image Cropping Library)
- **License**: Apache License 2.0
- **Source**: https://github.com/Yalantis/uCrop
- **Copyright**: Copyright 2017 Yalantis

uCrop is an image cropping library for Android.
Licensed under the Apache License, Version 2.0.

---

### CameraX
- **License**: Apache License 2.0
- **Source**: https://developer.android.com/jetpack/androidx/releases/camera
- **Copyright**: Copyright The Android Open Source Project

CameraX is a Jetpack support library for easier camera access.
Licensed under the Apache License, Version 2.0.

---

### Coil (Image Loading)
- **License**: Apache License 2.0
- **Source**: https://github.com/coil-kt/coil
- **Copyright**: Copyright 2021 Coil Contributors

Coil is an image loading library for Android backed by Kotlin Coroutines.
Licensed under the Apache License, Version 2.0.

---

### Glide (Image Loading)
- **License**: BSD, part MIT and Apache 2.0
- **Source**: https://github.com/bumptech/glide
- **Copyright**: Copyright 2014 Google LLC

Glide is a fast and efficient image loading library for Android.
Licensed under BSD-like license.

---

### Jetpack Compose
- **License**: Apache License 2.0
- **Source**: https://developer.android.com/jetpack/compose
- **Copyright**: Copyright The Android Open Source Project

Jetpack Compose is Android's modern toolkit for building native UI.
Licensed under the Apache License, Version 2.0.

---

### Material Design 3 Components
- **License**: Apache License 2.0
- **Source**: https://m3.material.io/
- **Copyright**: Copyright Google LLC

Material Design is a design system from Google.
Licensed under the Apache License, Version 2.0.

---

### Kotlin Coroutines
- **License**: Apache License 2.0
- **Source**: https://github.com/Kotlin/kotlinx.coroutines
- **Copyright**: Copyright JetBrains s.r.o.

Kotlin coroutines library for asynchronous programming.
Licensed under the Apache License, Version 2.0.

---

### AndroidX Libraries
- **License**: Apache License 2.0
- **Source**: https://developer.android.com/jetpack/androidx
- **Copyright**: Copyright The Android Open Source Project

AndroidX is the open-source project for developing Android apps.
Licensed under the Apache License, Version 2.0.

---

## License Texts

The Apache License 2.0 can be found at:
http://www.apache.org/licenses/LICENSE-2.0

A copy of the license is included in the LICENSE file in this repository.

---

## Acknowledgments

This application was created with the help of the open source community.
We thank all the contributors to these libraries for their excellent work.
